...
| --- | --- | --- |
| 自宅の開発用マシンの Linux | `home-dev-linux` | `home-dev-linux` |
...
nix 層の適用先は home-dev-linux と home-dev-wsl2 である。他の 3 つの環境では、パッケージの導入を本リポジトリが管理しない。

> [!IMPORTANT]
> home-dev-linux と home-dev-wsl2 では、chezmoi 層が用いる chezmoi と jq を nix 層が導入する。適用の順序は nix 層、chezmoi 層とすること。
